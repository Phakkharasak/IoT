...
void setup(void) {
  Serial.begin(115200);
  init_wifi("SAMSUNGPHAT", "1234567890");
  ...
  server.on("/", HTTP_GET, []() {
    ...
    html += "input[type='submit'] { background-color: #4CAF50; color: white; padding: 10px 20px; border: none; border-radius: 5px; cursor: pointer; font-size: 16px; width: 150px; margin: 10px auto; }";
    html += "input[type='submit']:hover { background-color: #45a049; }";
    html += "#turnOffButton { background-color: black; }";
    html += "</style>";
    html += "</head>";
    html += "<body><div>";
  html += "<h1 style='font-size: 70px;'>Web Server</h1>";
    ...
    if (digitalRead(ledPin) == HIGH) {
      ...
      html += "<script>var sensorEnabled = true;</script>";
    } else {
      html += "<span style='color: red;'>Off</span>";
      html += "<script>var sensorEnabled = false;</script>";
    }
    html += "</p>";
    ...
    html += "      } else {";
    html += "        document.getElementById('temperature').textContent = 'Sensor Disabled';";
    html += "        document.getElementById('humidity').textContent = 'Sensor Disabled';";
    html += "      }";
    html += "    }";
    html += "  };";
    html += "  xhr.send();";
